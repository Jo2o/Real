...
public class Main2 {
    ...
    public static void main(String[] args) {
        ...
        Map<Integer, Monkey> monkeys = createExampleMonkeys().stream()
        ...
        for (int round = 1; round <= ROUNDS; round++) {
            for (Entry<Integer, Monkey> entry : monkeys.entrySet()) {
                ...
                for (Item item : itemsOfCurrentMonkey) {
                    ...
                    int relievedWorryLevel = newWorryLevel;
        ...
        log.info("{}", monkeys.values().stream()
            .map(monkey -> monkey.getName() + ": " + monkey.getInspections())
            .collect(Collectors.toList()));
...
//        for (int i = 0; i < 2; i++) {
//            int inspections = inspectionOrderedMonkeys.get(i).getInspections();
//            log.info("Inspection {}: {}", i, inspections);
